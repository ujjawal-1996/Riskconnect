...
CompanyName       : Rackspace Inc
    ...
    .DESCRIPTION
    Custom QC script for Riskconnect (897998) that uses WHAM templates to configure servers per their AMG guidelines.
...
$text=ConvertTo-BBCode -Data $($WhamOutput | Select-Object *) | Out-String ;$text 
Add-CoreTicketUpdate -Text $text -TicketNumber $Ticket 
Set-RegValue -Ticket $Ticket -Key 'HKLM:\SYSTEM\CurrentControlSet\Services\W32Time\Parameters\' -Name 'NTPserver' -Type 'String' -Value '1069740-chvw709.stars-net.com,0x01 1069741-chvw710.stars-net.com,0x01' 
$text=ConvertTo-BBCode -Data $($WhamOutput | Select-Object *) | Out-String ;$text 
Add-CoreTicketUpdate -Text $text -TicketNumber $Ticket 
